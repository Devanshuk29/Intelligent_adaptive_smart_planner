...
const deleteGoal = async (req, res) => {
  try {
    ...
    console.log(`Deleting study sessions for goal ${goalId}...`);
    await pool.query(
      'DELETE FROM study_sessions WHERE goal_id = $1',
      [goalId]
    );

    console.log(`Deleting all the topics related to goalId ${goalId}...`);
    await pool.query(
      'DELETE FROM topics WHERE goal_id = $1',
      [goalId]
    )

    console.log(`Deleting tasks for goal ${goalId}...`);
    await pool.query(
      `DELETE FROM tasks 
       WHERE milestone_id IN (
         SELECT id FROM milestones WHERE goal_id = $1
       )`,
      [goalId]
    );

    console.log(`Deleting milestones for goal ${goalId}...`);
    await pool.query(
    ...
    console.log(`✅ Goal ${goalId} deleted successfully with all related data`);
...
module.exports = { createGoal, getAllGoals, getGoal, deleteGoal };
